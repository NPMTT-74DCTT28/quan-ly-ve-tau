...
// Lấy thông tin vé cần sửa
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    
    $sql_select = "SELECT * FROM ve_tau WHERE id = ?";
    $stmt_select = $conn->prepare($sql_select);
    $stmt_select->execute([$id]);
    $ve_tau = $stmt_select->fetch();
    
    if (!$ve_tau) {
        echo "<script>alert('Không tìm thấy vé tàu!'); window.location='index_ve_tau.php';</script>";
        exit();
    }
    ...
    // Lấy danh sách ghế cho lịch trình hiện tại
    ...
    echo "<script>alert('Không có ID vé tàu!'); window.location='index_ve_tau.php';</script>";
    exit();
}
...
if (isset($_POST['btnEdit'])) {
    ...
    // Kiểm tra mã vé đã tồn tại chưa (trừ vé hiện tại)
    ...
    if ($stmt_check->rowCount() > 0) {
        ...
        // Kiểm tra ghế đã được đặt chưa (trừ vé hiện tại)
        ...
        if ($stmt_check_ghe->rowCount() > 0) {
            ...
            if ($stmt_update->execute([$ma_ve, $id_khach_hang, $id_lich_trinh, $id_ghe, $id_nhan_vien, $gia_ve, $trang_thai, $id])) {
                echo "<script>
                    alert('Cập nhật thông tin vé tàu thành công!');
                    window.location='index_ve_tau.php';
                </script>";
                exit();
            } else {
...
<body>
    <div class="container">
        <div class="form-container">
            <h2>CẬP NHẬT THÔNG TIN VÉ TÀU</h2>
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control" name="id_lich_trinh" id="id_lich_trinh" required>
                    ...
                    <div class="col-12">
                        <div class="alert alert-warning">
                            Không có ghế nào khả dụng cho lịch trình này.
                        </div>
                    </div>
                    <?php endif; ?>
                </div>
